...
def get_doc_df(doc_id, bert_db_path):
    ...
    df = df.cumsum()
    if len(df) == 0:
        print(f"Not found doc {doc_id} in path {bert_db_path} - {df.shape}")
        assert len(df) > 0, f"Not found doc {doc_id} in path {bert_db_path}"
    return df.values
...
def __load_bert_db(doc_id):
    ...
    df = get_doc_df(doc_id, BERT_DB_PATH)

    _bert_db[doc_id] = df
    _bert_num_columns = df.shape[1]
...
def get_embedding(doc_id, begin, end):
    ...
    if begin == 0:
        return pd.Series(last_line / end)
