...
from extension.bins_features import load_bins
...
_bert_num_columns = 300  # 768  # *len(bins)
...
def __load_bert_db(doc_id):
    ...
    Embedder = import_helper.import_from_path("extension.embedder.BinsEmbedder")
    _bert_db[doc_id] = Embedder(get_doc_df(doc_id, bert_db_path), bins, eye)
...
def get_embedding(doc_id, begin, end, return_embedding=False, as_category=False):
    if begin >= end:
        return 0
    global _bert_db
    ...
    return np.asarray(_bert_db[doc_id].get_embedding(begin, end,
...
def bins_embedding_as_category(mention):
    def calc(mention):
        ...
        b = mention.span.begin
        e = mention.span.end + 1
        return get_embedding(doc_id, b, e, as_category=True)
