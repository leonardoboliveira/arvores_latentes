...
cdef class Perceptron:
    ...
    def fit(self, substructures, arc_information):
        ...
        print(f"init weights:{[x for x in self.weights['+'] if x != 0]}")
    ...
    cdef void _average_weights(self, double[:] weights, double[:] cached_weights,
        ...
        print(f"weights before avg:{[x for x in weights if x != 0]}")
        print(f"cached_weights:{[x for x in cached_weights if x != 0]}")
