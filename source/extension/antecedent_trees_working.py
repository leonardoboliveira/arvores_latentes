...
import numpy as np
import os
from extension.perceptron_working import Perceptron
...
class AntecedentTreePerceptron(Perceptron):
    ...
    def __init__(self,
        ...
        if isinstance(priors, int):
            ...
            for label in weights:
                weights[label] = np.multiply(weights[label], self.counter)
        else:
            self.counter = 0
    ...
    def fit(self, substructures, arc_information):
        ...
        # plt.show()
        print(f"Performance:{np.mean(self.perf)} +- {np.std(self.perf)}")
    ...
    def get_model(self):
        ...
        print(f"Priors:{model[0]}")
        weights = model[1]
        for label in weights:
            print(f"Final weights {[x for x in weights[label] if x != 0]}")

        return self.counter, model[1]
...
class AntecedentTreePerceptronWithMargin(AntecedentTreePerceptron):
    ...
    def find_best_arcs(self, arcs, arc_information, label="+"):
        ...
        margin = self.boost * 20000

        for arc in arcs:
            _, _, consistent = arc_information[arc]

            score = self.score_arc(arc, arc_information)

            score += margin * consistent
            ...
            if consistent and score > max_cons:
                best_cons = arc
                max_cons = score
