...
cdef class Perceptron:
    ...
    def __init__(self,
        ...
        from extension.cuda_aux import CudaAux
        self.cuda_aux = CudaAux()
        ...
        try:
            if weights is None:
                ...
                for label in weights:
                    ...
                    #print(f"Initial weights:{[x for x in weights_for_label if x != 0]}")
                    self.weights[label] = weights_for_label
    ...
    def fit(self, substructures, arc_information):
        ...
        #print(f"init Priors:{self.priors}")
        #print(f"init weights:{ {idx:x for idx,x in enumerate(self.weights['+']) if x != 0} }")
        cached_priors = {} #self.priors
        # cached_weights = {} #self.weights

        for label in self.priors:
            cached_priors[label] = self.priors[label] #0.0
        ...
        for epoch in range(1, self.n_iter+1):
            ...
            #print(f"update weights:{ {idx: x for idx, x in enumerate(self.weights['+']) if x != 0} }")
            logging.info("Finished epoch " + str(epoch))
            logging.info("\tIncorrect predictions: " + str(incorrect) + "/" + str(len(indices)))
            mean_perf = numpy.mean(self.perf) + numpy.std(self.perf)
            logging.info(f"\tPerformance:{mean_perf:.4f} +- {numpy.std(self.perf):.4f}")
            self.perf.clear()
            if ((mean_perf - max_perf) > 0.0001) or (incorrect < min_errors):
                logging.info("Improved")
                max_perf = max(max_perf, mean_perf)
                min_errors = min(min_errors, incorrect)
                not_improved = 0
            else:
                not_improved += 1
                logging.warn(f"Not improved {not_improved}")
                if not_improved >= 2:
                    logging.warn("Giving up")
                    break

        # averaging
        #for label in self.priors:
            #logging.info(f"Changing priors {self.counter} {(1/self.counter)*cached_priors[label]} [{cached_priors}]")
            #logging.info(f"Counter: {self.counter}")

            #self.priors[label] -= (1/self.counter)*cached_priors[label]
            #self._average_weights(self.weights[label], cached_weights[label], 1.0*self.counter)
    ...
    def predict(self, substructures, arc_information):
        ...
        for label in self.priors:
            ...
            self._average_weights(self.weights[label], self.cached_weights[label], 1.0*self.counter)

        #print(f"weights in use:{ { idx:x for idx,x in enumerate(self.weights['+']) if x != 0} }")
    ...
    def get_model(self):
        ...
        array_cached_weights = {}
        for label in self.cached_weights:
            array_cached_weights[label] = array.array("d", self.cached_weights[label])

        return self.counter, (array_weights, array_cached_weights)
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef void _update_cython_o(self,
                             double[:] weights,
    ...
    cdef void _update_cython(self,
                             ...
                             double update_val_for_weights,
                             double update_val_for_cached_weights):
        self.cuda_aux.update_vector(weights, cached_weights, nonnumeric_features, numeric_features, numeric_vals, numpy.double(update_val_for_weights), numpy.double(update_val_for_cached_weights))
    ...
    cdef void _average_weights(self, double[:] weights, double[:] cached_weights,
                          double counter):
        print(f"Average {counter}")
        cdef int index

        #print(f"weights before avg:{ {idx:x for idx,x in enumerate(weights) if x != 0} }")
        #print(f"cached_weights:{ {idx:x for idx,x in enumerate(cached_weights) if x != 0 } }")
