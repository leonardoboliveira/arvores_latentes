...
def plot(data,
    ...
    if colors is None:
        colors = cm.gray(numpy.linspace(0, 1, len(data)))
    ...
    for i, system_data in enumerate(data):
        ...
        for j, cat_and_number in enumerate(categories_and_numbers):
            ...
            hatch = patterns[i] if patterns is not None else None
            bar = ax.bar(2 * j + i * (1 / len(data)), number, color=colors[i],
                         width=1 / len(data), label=system_name, hatch=hatch, edgecolor="k")
