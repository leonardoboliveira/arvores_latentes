...
from pre_training.constants import MAX_CLASSES
...
def build_estimator(total_samples, num_epochs, model_dir, bert_dir, batch_size=None, train_all=True):
    ...
    bert_config = modeling.BertConfig.from_json_file(f"{bert_dir}/bert_config.json")
    init_checkpoint = f"{bert_dir}/bert_model.ckpt"
    if not os.path.isfile(f"{init_checkpoint}.meta"):
...
def input_fn_builder(input_files,
    ...
    def input_fn(params):
        ...
        name_to_features = {
            ...
            "masked_lm_ids":
                tf.FixedLenFeature(max_predictions_per_seq * MAX_CLASSES, tf.int64),
            "masked_lm_weights":
...
def get_masked_lm_output(bert_config, input_tensor, output_weights, positions,
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        numerator = tf.reduce_sum(label_weights * tf.reshape(per_example_loss, [-1]))
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        with tf.variable_scope("cls/predictions"):
            output_weights = tf.get_variable(
                "custom_output_weights", [MAX_CLASSES, bert_config.hidden_size],
                initializer=tf.truncated_normal_initializer(stddev=1))

        (masked_lm_loss, masked_lm_example_loss, masked_lm_logits) = get_masked_lm_output(
            bert_config, model.get_sequence_output(), output_weights,
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            eval_metrics = (metric_fn, [
                masked_lm_example_loss, masked_lm_logits, masked_lm_ids, masked_lm_weights
            ])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
